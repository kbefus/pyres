...
Example of a pyres implementation of "Surface electorde array 3" from the 
R2_v3.1_readme.pdf
...
ex_dir = r'C:\ER' 
work_dir = os.path.join(ex_dir,r'R2_examples\Surface_3\dpdp\Forward')
inv_work_dir = os.path.join(ex_dir,r'R2_examples\Surface_3\dpdp\Inverse')
...
r2_dict = {'survey_name':'Surface_3_dpdp',
...
npxstart,npystart = 1,5
nx_paramzone,ny_paramzone = 56,10 # number of elements in parameter zone
npx = 4*np.ones(nx_paramzone)
npy = 2*np.ones(ny_paramzone)
parameter_zone_dict = {'npxstart':npxstart,'npystart':npystart,
                       'npx':npx,'npy':npy}


inv_options = {'a_wgt':1e-3,'b_wgt':2e-2,
               'inverse_type':1,'patch_size_xy':[0,0],
               'rho_min':-1e3,'rho_max':1e3,
               }
inv_options.update(parameter_zone_dict) # insert parameter zone data, comment line for Surface_1_dpdp
...
plot_dict = {'work_dir':inv_work_dir,
             'region_xyz':np.array(target_xypos),
